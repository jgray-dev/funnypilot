...
device; that build is what left the car stuck at the boot logo with the comma
splash, unrecoverable by restarting. Everything else these features touch is
pure Python (plannerd, card) or Python + raylib (the UI), so nothing needs to
be compiled — and a small /dev/shm file gets the same two values across the
process boundary with zero build impact. controlsd already does exactly this
with /dev/shm/lat_interp, so this is the fork's established pattern, not a
new invention.

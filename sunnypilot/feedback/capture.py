...
class Capture:
  ...
  def sample(self, row):
    ...
    for event in self.active.values():
      if row['t'] <= event['mono_time'] + POST_SECONDS:
  ...
  def report(self, cmd, route, now=None):
    ...
    if event is None:
      ...
      with open(directory / 'telemetry.jsonl', 'w') as f:
        for row in self.ring:
          if cmd['started'] - PRE_SECONDS <= row['t'] <= now:
            f.write(json.dumps(row, allow_nan=False, separators=(',', ':')) + '\n')
      self.active[event_id] = event
    previous = set(event['labels'])
    event['labels'] = sorted(previous | set(cmd['labels']))
    if 'unnecessary_slowdown' in event['labels'] and 'unnecessary_slowdown' not in previous:
      ...
      if (c.get('governing') and not c.get('unmanageable') and
        ...
        rules[:] = [r for r in rules if not matches(r, c['lat'], c['lon'], c['bearing'])]
        rules.append({k:c[k] for k in ('lat','lon','bearing')} | {'event_id':event_id})
        self.rules['rules'] = rules[-MAX_RULES:]
        # Durable first, then publish. The planner never reads /data here.
        P.atomic_json(str(self.root / 'corners.json'), self.rules, durable=True)
        P.atomic_json(P.CORNER_RULES, self.rules)
  ...
  def finish(self, now=None):
    ...
    for event_id, event in list(self.active.items()):
      if now >= event['mono_time'] + POST_SECONDS:
        self._triage(event)
        event['state'] = 'queued'
        P.atomic_json(str(self.events / event_id / 'event.json'), event, durable=True)
  ...
  def _triage(self, event):
    ...
    with open(self.events / event['id'] / 'triage.jsonl', 'w') as out:
      for name in ('lat_interp.jsonl.1', 'lat_interp.jsonl'):
        try:
          with open('/data/funnypilot_triage/' + name) as source:

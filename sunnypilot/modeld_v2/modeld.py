...
class ModelState(ModelStateBase):
  ...
  def __init__(self, context: CLContext):
    ...
    # FunnyPilot v3.3.2: mirrors LAT_SMOOTH_SECONDS (bundle override only) —
    # the v3.2.12 adaptive smoothing is reverted; see main loop post-mortem.
  ...
  def get_action_from_model(self, model_output: dict[str, np.ndarray], prev_action: log.ModelDataV2.Action,
    ...
    desired_accel = smooth_accel(desired_accel, prev_action.desiredAcceleration, self.LONG_SMOOTH_SECONDS)
...
def main(demo=False):
  ...
  while True:
    ...
    # FunnyPilot v3.3.2: the v3.2.12 adaptive EMA + horizon shift is REVERTED.
    # Post-mortem: an EMA does not delay a maneuver, it REDISTRIBUTES it —
    # partial output appears through the window where the pre-3.2.12 command
    # was still flat before its decisive step ("a 2.5 in the space before the
    # 5"), i.e. steering onset creeps EARLIER however the sample horizon is
    # shifted. Not tunable away; concept rejected on-road. The lateral timing
    # returns to the validated scheme: knots sampled at the user's full delay
    # (bundle 'lat' override additive, as upstream), no adaptive smoothing —
    # the delay window is used by delay-compensated knots + LatSmoother's
    # in-period delta/5 interpolation in controlsd, and nothing else.
    lat_delay = model.lat_delay + model.LAT_SMOOTH_SECONDS
    model.lat_smooth_active = model.LAT_SMOOTH_SECONDS

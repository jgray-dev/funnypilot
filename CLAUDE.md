...
- Current development version: **`funnypilot-3.7.2`**. Its local checks passed;
  vehicle calibration/engagement validation was unavailable. Read that branch's
  instructions for details. Update this pointer when publishing a new version.
...
3. Choose the requested branch, or the current-development branch above if none
   ...
   ```bash
   BRANCH=funnypilot-3.7.2  # replace if a different version was requested

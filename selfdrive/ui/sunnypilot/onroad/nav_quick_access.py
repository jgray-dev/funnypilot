...
class NavQuickAccess(Widget):
  ...
  def update(self):
    ...
    self.visible = not in_drive

    # Check saved locations (only refresh occasionally — not every frame)
    home_raw = self._params.get("NavHomeLocation")
    work_raw = self._params.get("NavWorkLocation")
  ...
  def _on_tap(self, icon_type: str) -> None:
    params = self._params
    if icon_type == "home":
      raw = params.get("NavHomeLocation")
      if raw:
        try:
          data = json.loads(raw.decode() if isinstance(raw, bytes) else raw)
          params.put("NavDestination", json.dumps(data))
        except Exception:
          pass
    elif icon_type == "work":
      raw = params.get("NavWorkLocation")
      if raw:
        try:
          data = json.loads(raw.decode() if isinstance(raw, bytes) else raw)
          params.put("NavDestination", json.dumps(data))

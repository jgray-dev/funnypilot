...
class DriverMonitoring:
  ...
  def _get_distracted_types(self):
    ...
    if self.phone.prob_calibrated:
      using_phone = (
        self.phone.prob
        > max(min(self.phone.prob_offseter.filtered_stat.M, self.settings._PHONE_MAX_OFFSET), self.settings._PHONE_MIN_OFFSET) * self.settings._PHONE_THRESH2
      )
    else:
  ...
  def _update_states(self, driver_state, cal_rpy, car_speed, op_engaged, standstill, demo_mode=False):
    ...
    self.blink.left = (
      driver_data.leftBlinkProb * (driver_data.leftEyeProb > self.settings._EYE_THRESHOLD) * (driver_data.sunglassesProb < self.settings._SG_THRESHOLD)
    )
    self.blink.right = (
      driver_data.rightBlinkProb * (driver_data.rightEyeProb > self.settings._EYE_THRESHOLD) * (driver_data.sunglassesProb < self.settings._SG_THRESHOLD)
    )

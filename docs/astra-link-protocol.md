...
## Identity and safety
`identity`: `{branch: string|null, commit: string|null, dirty: boolean|null, version: string|null}` plus optional observation metadata. Dirty includes tracked/staged/untracked (`git status --porcelain --untracked-files=normal`, bounded); errors null. Approval compares ONLY these four fields, not timestamps. `state`: `{mode: 'offroad'|'onroad'|'unknown', reason: string, stationary: boolean|null, engaged: boolean|null}`. Mode remains descriptive; ignition-on/onroad alone does not block operations. Commands require `stationary === true` and `engaged === false`; missing fields from older clients normalize to null and cannot authorize commands. Native motion requires valid CAN, standstill, and finite filtered/raw speeds below 0.01 m/s in absolute value. The existing FunnyPilot feedback consumer publishes this nullable stationary result and the original carState consumer-monotonic receive time to `/dev/shm/fp_motion.json` at most 5 Hz; Astra reads at most 512 bytes and does not allocate another carState reader. This tmpfs snapshot clears on reboot; absent/malformed, future-dated, nonfinite, or over-two-second-old observations cannot authorize. Republish never refreshes the original sample time. Engagement includes selfdrive enabled/active, MADS enabled/active, and carControl enabled/lateral/longitudinal activity. These services must be seen, valid, and received within two consumer-monotonic seconds; stale/absent evidence is null, even offroad. Server/browser approval requires a heartbeat under 10 seconds old. Device independently enforces native freshness throughout commands. Network is not responsible for refreshing native safety state.
...
- POST `/api/linked-devices/approve` `{id,digest}` -> `{ok:true}`. Only pending commands; device freshly online, stationary and disengaged, with unchanged generation/identity; binds approver browser ID, 120s expiry. Reject approval on browser revocation/expiry at grant/dispatch. Approval is not native permission-menu input.
